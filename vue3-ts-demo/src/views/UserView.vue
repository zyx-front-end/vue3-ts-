    ...
    <el-dialog v-model="isShow" title="编辑信息">
      <el-form :model="active">
        ...
        <el-form-item label="角色" label-width="50px">
          <el-select multiple v-model="active.role" placeholder="请选择相应的角色">
            <el-option
              v-for="item in roleList"
              :key="item.roleId"
              :label="item.roleName"
  ...
  setup() {
    ...
    const onSubmit = () => {
      ...
      if (data.selectData.nickName || data.selectData.role) {
        ...
        if (data.selectData.role) {
          arr = data.selectData.nickName ? arr : data.list;
    ...
    const changeUser=(row:ListInt)=>{
        ...
        data.active={
            ...
            role : row.role && row.role.map((value:any) => value.role || value.roleId)
        }
        data.isShow=true
